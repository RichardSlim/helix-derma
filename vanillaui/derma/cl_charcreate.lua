...
function PANEL:Init()
	...
	descriptionProceed.DoClick = function()
		if (self:VerifyProgression("description")) then
			self.progress:IncrementProgress()
			self:Populate()
			self:SetActiveSubpanel("skins")
		end
	end

	-- Skin Subpanel -TheLife
	self.skins = self:AddSubpanel("skins")
	self.skins:SetTitle("Choose skin")

	local skinsModelList = self.skins:Add("Panel")
	skinsModelList:Dock(LEFT)
	skinsModelList:SetSize(halfWidth, halfHeight)

	local skinsBack = skinsModelList:Add("ixMenuButton")
	skinsBack:SetText("return")
	skinsBack:SetContentAlignment(4)
	skinsBack:SizeToContents()
	skinsBack:Dock(BOTTOM)
	skinsBack.DoClick = function()
		self.progress:DecrementProgress()
		self:SetActiveSubpanel("description")
	end
	
	self.skinsModel = skinsModelList:Add("ixModelPanel")
	self.skinsModel:Dock(FILL)
	self.skinsModel:SetModel(self.descriptionModel:GetModel())
	self.skinsModel:SetFOV(modelFOV - 13)
	self.skinsModel.PaintModel = self.skinsModel.Paint

	self.skinsPanel = self.skins:Add("Panel")
	self.skinsPanel:SetWide(halfWidth + padding * 2)
	self.skinsPanel:Dock(RIGHT)

	local skinsProceed = self.skinsPanel:Add("ixMenuButton") 
	skinsProceed:SetText("proceed")
	skinsProceed:SetContentAlignment(6)
	skinsProceed:SizeToContents()
	skinsProceed:Dock(BOTTOM)
	skinsProceed.DoClick = function()
		self.progress:IncrementProgress()
	...
	self.progress:SetFont("cwMenuButtonFontSmall")
	...
	self:AddPayloadHook("model", function(value)
		...
		if (faction) then
			...
			if (istable(model)) then
				...
				self.skinsModel:SetModel(model[1], model[2] or 0, model[3])
				...
				self.skinsModel:SetModel(model)
...
function PANEL:GetDefaultBodygroups(faction, model)
	local allModels = faction:GetModels(LocalPlayer())
	for k,v in pairs(allModels) do
		if (istable(v)) then
			if (model == v[1]) then
				return v[3]
			end
...
function PANEL:Populate()
	...
	for k, v in SortedPairsByMemberValue(ix.char.vars, "index") do
		if (!v.bNoDisplay and k != "__SortedIndex") then
			...
			elseif (isstring(v.default)) then
				...
				panel:Dock(TOP)
				panel:SetFont("cwMenuButtonFontLarge")
			...
			if (IsValid(panel)) then
				...
				label:SetFont("cwMenuButtonFont")
	...
	do
		-- Set up skins
		local model = self.skinsModel:GetModel()
		local faction = ix.faction.indices[self.payload.faction]
		local bodygroups = self:GetDefaultBodygroups(faction, model)
		local panel
		local container = self:GetContainerPanel("skins")
		
		if (model) then
			panel = self:GetSkinsPanel(container, model, bodygroups)
			...
			if (IsValid(panel)) then
				...
				self:AttachCleanup(label)
				self:AttachCleanup(panel)
				local vars = ix.char.vars.model
				if (vars.OnPostSetup) then
					vars:OnPostSetup(panel, self.payload)
				end
...
function PANEL:GetSkinsPanel(container, model, bodygroups)
	local skins = NumModelSkins(model)
	local faction = ix.faction.indices[self.payload.faction]
	...
	for i=0, skins-1 do
		...
		icon.DoClick = function(this)
			self.payload.Set("skin", i)
			self:Populate()
			self.skinsModel:SetModel(self.skinsModel:GetModel(), i, self:GetDefaultBodygroups(faction, self.skinsModel:GetModel()))
			net.Start("RGSetCharacterCreationSkin")
			net.WriteInt(i, 6)
			net.SendToServer()
		end
		icon.PaintOver = function(this, w, h)
			if (self.payload.skin and self.payload.skin == i) then
		...
		icon:SetModel(model, i, bodygroups)
	end
	return scroll
end
