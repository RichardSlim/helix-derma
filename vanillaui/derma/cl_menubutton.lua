...
function PANEL:Init()
	...
	--self.padding = {2, 2, 2, 2} -- left, top, right, bottom
	self.backgroundColor = Color(0, 0, 0)
	self.backgroundAlpha = 128
	self.currentBackgroundAlpha = 0
end
...
function PANEL:SetDisabled(bValue)
	...
	if (bValue) then
		self:SetTextColorInternal(Color(math.max(color.r - 150, 0), math.max(color.g - 150, 0), math.max(color.b - 150, 0)))
	else
		self:SetTextColorInternal(color)
	end
...
function PANEL:OnCursorEntered()
	...
	local color = self:GetTextColor()
	self:SetTextColorInternal(Color(math.max(color.r - 50, 0), math.max(color.g - 50, 0), math.max(color.b - 50, 0)))
...
vgui.Register("cwMenuButton", PANEL, "DButton")